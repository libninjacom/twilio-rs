...
#[derive(Clone)]
pub struct ListAccountRequest<'a> {
...
impl<'a> ListAccountRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self.http_client.client.get("/2010-04-01/Accounts.json");
...
impl<'a> ::std::future::IntoFuture for ListAccountRequest<'a> {
