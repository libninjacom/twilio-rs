...
#[derive(Clone)]
pub struct ListMemberRequest<'a> {
...
impl<'a> ListMemberRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/Queues/{queue_sid}/Members.json",
                    account_sid = self.account_sid, queue_sid = self.queue_sid
                ),
            );
...
impl<'a> ::std::future::IntoFuture for ListMemberRequest<'a> {
