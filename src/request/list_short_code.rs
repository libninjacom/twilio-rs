...
#[derive(Clone)]
pub struct ListShortCodeRequest<'a> {
...
impl<'a> ListShortCodeRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/SMS/ShortCodes.json", account_sid
                    = self.account_sid
                ),
            );
    ...
    pub fn short_code(mut self, short_code: &str) -> Self {
        self.short_code = Some(short_code.to_owned());
        self
    }
}
impl<'a> ::std::future::IntoFuture for ListShortCodeRequest<'a> {
