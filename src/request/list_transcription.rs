...
#[derive(Clone)]
pub struct ListTranscriptionRequest<'a> {
...
impl<'a> ListTranscriptionRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/Transcriptions.json", account_sid
                    = self.account_sid
                ),
            );
...
impl<'a> ::std::future::IntoFuture for ListTranscriptionRequest<'a> {
