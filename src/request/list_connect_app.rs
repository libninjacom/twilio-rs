...
#[derive(Clone)]
pub struct ListConnectAppRequest<'a> {
...
impl<'a> ListConnectAppRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/ConnectApps.json", account_sid =
                    self.account_sid
                ),
            );
...
impl<'a> ::std::future::IntoFuture for ListConnectAppRequest<'a> {
