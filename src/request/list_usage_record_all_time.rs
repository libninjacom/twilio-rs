...
#[derive(Clone)]
pub struct ListUsageRecordAllTimeRequest<'a> {
...
impl<'a> ListUsageRecordAllTimeRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/Usage/Records/AllTime.json",
...
impl<'a> ::std::future::IntoFuture for ListUsageRecordAllTimeRequest<'a> {
