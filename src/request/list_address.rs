...
#[derive(Clone)]
pub struct ListAddressRequest<'a> {
...
impl<'a> ListAddressRequest<'a> {
    pub async fn send(self) -> ::httpclient::InMemoryResult<serde_json::Value> {
        let mut r = self
            ...
            .get(
                &format!(
                    "/2010-04-01/Accounts/{account_sid}/Addresses.json", account_sid =
                    self.account_sid
                ),
            );
        if let Some(ref unwrapped) = self.customer_name {
            r = r.query("CustomerName", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.friendly_name {
            r = r.query("FriendlyName", &unwrapped.to_string());
        }
        if let Some(ref unwrapped) = self.iso_country {
            r = r.query("IsoCountry", &unwrapped.to_string());
        }
    ...
    pub fn customer_name(mut self, customer_name: &str) -> Self {
        self.customer_name = Some(customer_name.to_owned());
        self
    }
    ...
    pub fn iso_country(mut self, iso_country: &str) -> Self {
        self.iso_country = Some(iso_country.to_owned());
        self
    }
...
impl<'a> ::std::future::IntoFuture for ListAddressRequest<'a> {
